...
type DismissGuard = Rc<dyn Fn(&MouseDownEvent, &Window, &App) -> bool>;
...
pub struct Popover {
    ...
    dismiss_guard: Option<DismissGuard>,
    on_open_change: Option<OpenChangeHandler>,
}
...
impl Popover {
    ...
    /// Allows a styled facade to keep an outside press owned by one of its
    /// deferred descendants from dismissing the popover first.
    #[doc(hidden)]
    pub fn dismiss_guard(
        mut self,
        guard: impl Fn(&MouseDownEvent, &Window, &App) -> bool + 'static,
    ) -> Self {
        self.dismiss_guard = Some(Rc::new(guard));
        self
    }
    ...
    fn render(self, window: &mut Window, cx: &mut App) -> impl IntoElement {
        let state = window.use_keyed_state(self.id.clone(), cx, |_, cx| {
            cx.on_release(|state: &mut PopoverState, cx| {
                GlobalState::unregister_deferred_popover(&state.focus_handle, cx);
            })
            .detach();
        ...
        let content = div()
            ...
            .when(self.overlay_closable, |this| {
                this.on_mouse_down_out({
                    let state = state.clone();
                    let dismiss_guard = self.dismiss_guard.clone();
                    move |event, window, cx| {
                        if dismiss_guard
                            .as_ref()
                            .is_some_and(|guard| !guard(event, window, cx))
                        {
                            return;
                        }
                        state.update(cx, |state, cx| state.dismiss(window, cx));
